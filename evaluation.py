...
from sklearn.metrics import classification_report, f1_score, recall_score, accuracy_score,mean_absolute_error, mean_squared_error, r2_score
...
def model_eval_sst(dataloader, model, device):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc=f'eval', disable=TQDM_DISABLE)):
        ...
        #CHANGED**** @ 23.12.2023
        #logits,_ = model.predict_sentiment(b_ids, b_mask,True)
        #CHANGED****
    ...
    #CHANGED****  @ 27.12.2023
    target_names = [f"class_{i}" for i in range(5)]                                                                       # get target names
    print("Sentiment analysis:: ",classification_report(y_true, y_pred, target_names=target_names,zero_division=0.0))     # print f1 score, precision, recall
    #CHANGED****
...
def model_eval_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        #CHANGED**** @ 27.12.2023
        target_names = [f"class_{i}" for i in range(2)]                                                                                 # get target names
        print("Paraphrase detection:: ",classification_report(para_y_true, para_y_pred, target_names=target_names,zero_division=0.0))   # print f1 score, precision, recall
        #CHANGED****
        ...
        #CHANGED**** @ 27.12.2023
        mae = mean_absolute_error(sts_y_true, sts_y_pred)                             # calculate mean absolute error
        mse = mean_squared_error(sts_y_true, sts_y_pred)                              # calculate mean squared error
        r2 = r2_score(sts_y_true, sts_y_pred)                                         # R2 score
        print(f"Sementic textual similarity:: MSE: {mse}, MAE: {mae}, R2: {r2}")      #print mse, mae, r2 metrics
        #CHANGED****
        ...
        #CHANGED**** @ 27.12.2023
        target_names = [f"class_{i}" for i in range(5)]                                                                             # get target names
        print("Sentiment analysis:: ",classification_report(sst_y_true, sst_y_pred, target_names=target_names,zero_division=0.0))   # print f1 score, precision, recall
        #CHANGED****
